...
class VideoTrack extends MediaTrack {
  ...
  constructor(mediaTrackTransceiver, options) {
    ...
    Object.defineProperties(this, {
      ...
      dimensions: {
        enumerable: true,
        value: {
          width: options?.video?.width || 0,
          height: options?.video?.height || 0,
        },
      },
      frameRate: {
        enumerable: true,
        value: options?.video?.frameRate || DEFAULT_FRAME_RATE,
      },
      processor: {
        enumerable: true,
        value: null,
    ...
    this._processorEventObserver = new (options.VideoProcessorEventObserver ||
      VideoProcessorEventObserver)(this._log);
    ...
    this._dummyEl
      ...
      .then(() => {
        const process = videoFrame => {
          ...
          const { width = 0, height = 0 } = this._getSettings();
          ...
          if (this._inputFrame) {
            ...
            this._inputFrame
              .getContext('2d')
              .drawImage(this._dummyEl, 0, 0, width, height);
          }
      ...
      .catch(error =>
        this._log.error('Video element cannot be played', {
          error,
          track: this,
        })
      );
  ...
  addProcessor(processor, options) {
    ...
    let { inputFrameBufferType, outputFrameBufferContextType } =
    ...
    if (
      ...
      throw new Error(
        `Invalid inputFrameBufferType of ${inputFrameBufferType}`
      );
    }
    if (!inputFrameBufferType) {
      inputFrameBufferType =
        typeof OffscreenCanvas === 'undefined' ? 'canvas' : 'offscreencanvas';
    }
    ...
    if (
      ...
      this._log.warn(
        'outputFrameBufferContextType = "bitmaprenderer" is not supported by ' +
    ...
    if (!ctx) {
      throw new Error(
        `Cannot get outputFrameBufferContextType: ${outputFrameBufferContextType}.`
      );
    }
